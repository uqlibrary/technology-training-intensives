...
Pandas v3.0 changes the (default) behaviour for string columns. They will no longer be `object` dtypes, but `str` dtypes (by default). Most features will work the same, but it may cause some breaking changes: see the [migration user guide](https://pandas.pydata.org/pandas-docs/stable/user_guide/migration-3-strings.html) for details.
