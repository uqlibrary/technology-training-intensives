...
format: dashboard
warning: false
categories: [R, 25Summer, "data: melb_data.csv"]
---
##
...
#| width: 30%
library(knitr)
table1 <- read.csv("gallery/R/25Summer/Karunarathna/Tables/tables1_1.csv")
kable(table1)
```
...
#| width: 70%
# library(magick)
table2 <- read.csv("gallery/R/25Summer/Karunarathna/Tables/table2.csv")
kable(table2)
```
...
housing <- read.csv("data/melb_data.csv")
...
#| width: 30%
# library(magick)
table3 <- read.csv("gallery/R/25Summer/Karunarathna/Tables/table3.csv")
kable(table3)
```
