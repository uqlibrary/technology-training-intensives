...
if (number < 5) {
  paste(number, "is less than 5.")
}
```

> The `paste()` function lets you print multiple things together

What if we wanted to check all the numbers from 1 to 4? We can use a while loop.

```{r}
number <- 1

while (number < 5) {
  print(paste(number, "is less than 5."))
  number <- number + 1
}
```

> We need to `print()` the output of `paste()` because we're doing it multiple times. R by default only prints to the console the value returned by the last command.
