...
* The main features of the Positron IDE
...
The **console** (by default at the bottom in Positron) is where most of the action happens. In the console, we can use R interactively. We write a **command** and then **execute** it by pressing <kbd>Enter</kbd>.
...
You should now see your objects listed in you **Session pane** (top right).
...
You can use whatever object names you like, it doesn't have to be "num1" or "sentence", but you will want to be clear and consistent in the kinds of names you use, so you don't make typos and know what your objects contain.
...
You can do that with the "Explorer" panel opened from the Activity Bar on the left.
...
Let's create a new R script, from the same Explorer panel.

This opens another pane at the top of Positron: the **Editor pane**.
...
```r
# Description: Introduction to R and Positron
# Author: <your name>
# Date: <today's date>
```
...
Notice the colours? This is called **syntax highlighting**. This is one of the many ways Positron makes it more comfortable to work with R. The code is more readable when working in a script.

> While editing your script, you can run the current command (or the selected block of code) by using <kbd>Ctrl</kbd>+<kbd>Enter</kbd>. Remember to save your script regularly with the shortcut <kbd>Ctrl</kbd>+<kbd>S</kbd>.
...
There are two main ways to find **help** about a specific function in Positron:
...
Errors and warnings are a normal part of coding, and nothing to be afraid of - they're there to help you!
However, it can be frustrating when the error is confusing, or doesn't seem to make sense. 

Warnings won't stop your code from running, but it's often worth investigating to make sure it's behaving the way you expect.

Errors will stop your code, and will hopefully be accompanied by a message that makes sense.
Most of the time errors are caused by simple typos, like the incomplete functions above.

Typos
```r 
> man(c(1, 2, 3, 3, 10))
Error in man(c(1, 2, 3, 3, 10)) : could not find function "man"
```
...
> num1 <- 42
> num2 <- Num1 / 9
Error: object 'Num1' not found
```

Missing Commas
```r
> ages <- c(4, 10 2, NA, 3)
Error: unexpected numeric constant in "ages <- c(4, 10 2"
```

Missing Quotes
```r
> barplot(ages, main = "How old are my dogs?, col = "pink")
Error: unexpected symbol in "barplot(ages, main = "How old are my dogs?, col = "pink"
```
...
Let's bring in some data. [Download our gapminder dataset](../../data/gapminder.csv) and save it in the `data/` folder you just created.
...
> The Positron "Session" panel already shows us some of that information (click on the greater-than symbol next to the object name).

And to explore the data in a viewer, click on the table icon next to the object in the Session pane (it opens in a new tab next to your script).

This viewer allows you to explore your data by scrolling through, searching terms, filtering rows and sorting the data. It also shows quick stats and histograms. Remember that it is only a viewer: it will never modify your original object.

> Notice that Positron actually runs the `View()` function. Feel free to use that instead of clicking on the button, but note that the case matters: using a lowercase "v" will yield an error.
...
Packages are extensions to R, which usually include extra functions, but also datasets. There are [more than 24,000](https://cran.r-project.org/web/packages/index.html) available.

You can see the list of installed packages in your "Packages" panel, or by using the `library()` function without any argument.

We are going to install a package called "skimr". We can do that in the Packages panel:

1. Open the "Packages" panel from the Activity bar
1. Click the three-dot menu and click “Install”
1. Type “skimr” and press Enter to search
1. Select "skimr" and the latest version suggested
...
* Can you make a chicken say "bok" a thousand times? (Hint: look at the `rep.int()` and `paste()` functions and their arguments.)
