...
-   Click the `Project Home` dropdown, then `data > raw > 5-styling_data` and load in:
-   artefacts.gpkg (artefact location data we've made for this tutorial)

## Aims for this session

We've been sent through the data from the cultural heritage survey. We want to put this map into a report for the public.
...
-   A good looking map for a generic **Report** map may look different to an **Academic Paper** or a **Technical** map or to a map for the **Public**.
...
Let's start by turning our map into one we can use in our **Report for the Public**, where we want to focus their attention on our data. Unless we really *need* **Satellite** imagery, or everything on the **OpenStreetMap**, they're both too busy and distracting.
...
Arrange the layers so that the **Voyager (no labels)** layer is under your other layers (you can hide the **OpenStreetMap** for now).
...
We can use the `Style Manager` to edit the default options and Favorites that appear in the Layer Styling window.

-   Navigate to `Settings > Style Manager...`

    -   Click the ![](https://docs.qgis.org/3.40/en/_images/symbologyAdd.png) `Add item` button at the bottom and select `Catalog: cpt-city...`

        -   From the left panel under `Name`, select `Topography`
        -   From the right panel under `Selected palettes for Topography`, scroll down click `wiki-knutux`, and click `OK`
        -   Set the `Name` as "Knutux" (or something that makes sense to you), tick `Add to my favorites`, and click `Save`
    -   Click `Close`

You can do this same process for any other colour ramp that you like. You can also create custom colour ramps of your choosing.

:::  {.callout-tip collapse="true"}
### Custom Colours

If you want to be really fancy, you can import and build our own Color Ramp and Color Palette.

-   In your internet browser, navigate to [colorbrewer2.org](https://colorbrewer2.org/#type=qualitative&scheme=Set2&n=7)
...
-   Back in QGIS navigate to `Settings > Options...`
    ...
    -   If you created a custom palette earlier, from the top right of the window click the `...` button and select `New Palette...`, name it **Report Palette**

    -   Add your copied colours by clicking![](https://docs.qgis.org/3.40/en/_images/mActionEditPaste.png) `Paste colors` on the right
...
-   Navigate to `Settings > Style Manager...`
    ...
    -   Click the ![](https://docs.qgis.org/3.40/en/_images/symbologyAdd.png) `Add item` button at the bottom and select `Color Presets...`

        -   Select the colour that appeared there and remove it by pressing ![](https://docs.qgis.org/3.40/en/_images/symbologyRemove.png) `Remove color` at the bottom

        -   Add your copied colours by clicking![](https://docs.qgis.org/3.40/en/_images/mActionEditPaste.png) `Paste colors`
...
Now that we have added that colour ramp to our favourites, we can more easily apply it to our layers.

#### Categories

-   Select the **Artefacts** layer from the `Layer Styling` panel
-   In the `Layer Styling` panel, change **`Single Symbol`** to `Categorized`
-   From `Value` select **Artefact_Type**
-   From `Color ramp` choose our **Knutux**
-   Click the **`Classify`** button

#### Symbols

With the **Voyager (no labels)** basemap, we can reduce the complexity of our symbology to make a cleaner looking map. Let's keep styling the **Artefacts**

If you want, you can change the properties of all symbols at once:

-   Click in the white space below your symbol classes so that no categories are highlighted/selected.
...
By removing the fill from the points it reduces the visual noise from our map, and can reduce the effects of overplotting.
...
-   Click the ![](https://docs.qgis.org/3.40/en/_images/symbologyAdd.png) `Add symbology layer` next to `Fill` to add another fill symbol\
...
-   Click the new **Simple Fill**
    ...
    -   Scroll to the bottom, tick `Draw Effects`, then click the ![](https://docs.qgis.org/2.18/cs/_images/mIconPaintEffects.png) `Customise effects` button (The one next to "Enable symbol layer", *not* the one in the "Layer Rendering" section!)
        -   Untick `Source`
...
In this case, we want to keep the attention on the point data, let's keep the tracks, but reduce their contrast.

-   Select the **Access Tracks** layer from the `Layer Styling` panel
-   Under `Line`, click `Simple Line`
    -   If you created a custom palette earlier, change the `Color` to a colour in your **Report Palette** that is different to your points, I'm going to use **green**
...
-   In the `Layers` panel, hide the **Voyager (no labels)** layer by unticking the box next to it.

-   Unhide the **World Imagery**
...
-   In the `Layers` panel, hide the **World Imagery**, **Access Tracks** and **Artefacts** layers by unticking the boxes next to them.

-   Unhide the **Voyager (no labels)**

-   Click Manage Map Themes ![Manage Map Themes Icon](https://docs.qgis.org/3.40/en/_images/mActionShowPresets.png) at the top of the `Layers` panel, and click `Add theme...`
...
-   Click ![](https://docs.qgis.org/3.40/en/_images/symbologyAdd.png) `Add new guide` under **Vertical Guides** (if your canvas is a Landscape, or you want your legend at the bottom, choose **Horizontal Guides** for this step)
...
-   Navigate back to your **Project Map** window
    ...
    -   You will see these options: ![](https://docs.qgis.org/3.40/en/_images/mActionSetToCanvasExtent.png)![](https://docs.qgis.org/3.40/en/_images/mActionViewExtentInCanvas.png)![](https://docs.qgis.org/3.40/en/_images/mActionSetToCanvasScale.png)![](https://docs.qgis.org/3.40/en/_images/mActionViewScaleInCanvas.png)
...
-   In the right menu, click `Guides`
    -   Click ![](https://docs.qgis.org/3.40/en/_images/symbologyAdd.png) `Add new guide` under **Horizontal Guides** *twice*
...
-   In the `Item Properties` panel, click the box next to `Style`, this will take you to the `Symbol Settings` window
    -   Click **Simple Fill**
        ...
        -   Scroll to the bottom, tick `Draw Effects`, then click the ![](https://docs.qgis.org/3.40/en/_images/mIconPaintEffects.png) `Customise effects` button
            -   Untick `Source`
...
-   Navigate back to QGIS (it's still open in another window)
    ...
    -   From the top toolbars open the ![](https://docs.qgis.org/3.40/en/_images/mActionCalculateField.png) `Field Calculator`
...
-   Navigate back to your **Layout Manager** window
    ...
    -    ![](https://docs.qgis.org/3.40/en/_images/mActionSetToCanvasExtent.png)![](https://docs.qgis.org/3.40/en/_images/mActionViewExtentInCanvas.png)![](https://docs.qgis.org/3.40/en/_images/mActionSetToCanvasScale.png)![](https://docs.qgis.org/3.40/en/_images/mActionViewScaleInCanvas.png)
    -   Click the first orange arrow button to **Set Map Extent to Match Main Canvas Extent**
    -   Click the third orange arrow button to **Set Map Scale to Match Main Canvas Scale**
        -   The other buttons do the reverse
