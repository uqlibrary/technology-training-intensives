...
- Try [**Zonal Statistics**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#zonal-statistics) to calculate average population or elevation within service catchments.
- Explore [**Service Area (Network Analysis)**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/networkanalysis.html#service-area-from-layer) for more realistic travel distances.
:::
...
- Try [**Zonal Statistics**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#zonal-statistics) to calculate average population that may be affected.
- Use [**Raster Calculator**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#raster-calculator) to combine multiple risk layers.  
- Try [**Extract by Mask**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#extract-by-mask) to isolate high‑risk zones.
:::
...
- Use [**Zonal Statistics**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#zonal-statistics) to summarise raster values by land use type.  
- Try [**Spatial Join (Summary)**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#join-attributes-by-location-summary) to attach population counts to land use zones.
...
- Use [**Raster Calculator**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#raster-calculator) to combine heat and vegetation layers.
- Try [**Line Density**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#line-density) on footpaths to find heavily exposed walking routes.
:::
...
- Use [**Zonal Statistics**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#zonal-statistics) to summarise habitat quality by area.  
- Try [**Proximity (Distance to Nearest Hub)**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#distance-to-nearest-hub) to measure distance to roads or towns.
:::
...
- Use [**Raster Calculator**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#raster-calculator) to weight suitability factors.  
- Try [**Multi‑criteria Evaluation (simple overlay)**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#weighted-overlay) with reclassified rasters.
:::
...
- Use [**Kernel Density Estimation**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#heatmap-kernel-density-estimation) for smoother patterns.  
- Try [**Nearest Neighbour Analysis**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#nearest-neighbour-analysis) for clustering insight.
:::
...
- Use [**Spatial Join (Summary)**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#join-attributes-by-location-summary) to link observations to areas.  
- Try [**Point Density**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/vectoranalysis.html#point-density) to highlight hotspots.
:::
...
- Use [**Raster Calculator**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#raster-calculator) to combine slope and distance.  
- Try [**Network Shortest Path**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/networkanalysis.html#shortest-path-point-to-point) for simple routing.
:::


::: {.callout-tip collapse="true"}
### Scenario 10: Archaeological Sensitivity and Cultural Landscape Assessment
...
- Use the [**Raster Calculator to reclassify**](https://docs.qgis.org/3.40/en/docs/training_manual/rasters/terrain_analysis.html#follow-along-reclassifying-the-raster) and combine environmental predictors into a single sensitivity surface.
- Use [**Zonal Statistics**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#zonal-statistics) to summarise sensitivity values within development parcels or survey areas.  
- Explore [**Proximity (Raster Distance)**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#proximity-distance) as an input to a simple predictive model.  
:::

::: {.callout-tip collapse="true"}
### Scenario 11: A Place-Based Story of your Choosing
...
- Try [**Zonal Statistics**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#zonal-statistics) or [**Raster Calculator**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#raster-calculator) for deeper analysis.  
- Explore [**Viewshed**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#viewshed) or [**Slope**](https://docs.qgis.org/latest/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html#slope) for terrain‑based stories.
:::
...
GeoScience have a a varierty of [OGC Webservice Data](https://knowledge.dea.ga.gov.au/data/) that you can access Earth and Envrionment related rasters in the same way we used WebMapping Services to access Basemaps.

### Climate Rasters

The [Bureau of Meterology](https://www.bom.gov.au/climate/maps/averages/) has many types of Grid data of climate data (Rainfall, Temperature, Solar Exposure, etc) that you can download and import into QGIS.

### Population Rasters

The ABS have a lot of useful data, including population data. They provide it in excel format, as GeoPackages (by SA2 and LGAs), and as [population grid raster files](https://www.abs.gov.au/statistics/people/population/regional-population/latest-release#data-download).
...
You can search for data on [data.gov](https://data.gov.au/data/dataset/), and then filter the formats only for spatial files like SHP, tif, GEOTIFF, GeoJSON, KML, KMZ, GDB, FGDB, GeoPackage, NetCDF, ASC, REST, WCS, ESRI, ArcGIS, WFS, WMS, WMTS, WCM, etc.
  
#### Digital Atlas of Australia
Explore data similar to data.gov, but explcitly for spatial data and interactive maps with the [Digital Atlas of Australia](https://digital.atlas.gov.au)
...
You can also find tabular data separated by state containing information like [Crime Data](https://www.abs.gov.au/statistics/people/crime-and-justice/recorded-crime-offenders/latest-release#data-downloads)
:::
