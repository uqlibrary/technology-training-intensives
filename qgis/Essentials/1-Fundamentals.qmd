...
-   the `Canvas`, where we see the map (at startup, QGIS uses that space to show recent projects and QGIS news)
...
-   Let's **save** our project: `Project > Save`.
...
-   In the **Processing Toolbox** panel, `Search` for "Fix geometries"
...
3.  Use the bvg1m column in the "Value" field
...
2.  Choose `Draw on Canvas` and draw the area you want to export

3.  Increase the `Resolution` to 300 DPI

4.  Click `Save`

5.  Navigate to where you want to save your image (we created the "output" folder for that), name it, and click `Save`

## Resources

-   You can find help in the [QGIS Documentation](https://docs.qgis.org/3.40/en/docs/index.html)
