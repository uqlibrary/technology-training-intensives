...
You can also use the `Project Home` directory in the browser to add another layer: try loading the "Project_Area.gpkg" and "Access_Track.gpkg" GeoPackages (format .gpkg), by drag-and-dropping or double-clicking.
...
We'll use the Clip tool on the final piece of data for this session: the [Broad Vegetation Groups (BVGs)](https://www.qld.gov.au/environment/plants-animals/plants/ecosystems/broad-vegetation) for this area. BVG data shows us what vegetation types are found in a particular location. We've downloaded a section of this data from [QSpatial](https://qldspatial.information.qld.gov.au/catalogue/custom/detail.page?fid={43A2CB31-9D83-4BB9-ACE7-05E7BD271FE3}). Currently it covers a wide area, but we want to focus it down to our project.
...
Double-click on the **Project Area** layer to open the `Layer Properties` click the ![](https://docs.qgis.org/3.40/en/_images/symbology.png) `Symbology` tab, and use `Color` to change to a better colour. Click `Apply`. Perhaps we'd be better off with no fill, and work more with the border by clicking on `Simple fill`. Change the `Fill Style` to `No Brush`. We can now go into more details about the **Stroke** color, width and style.
...
2.  Change from "Single Symbol" to "Categorised"
3.  Use the bvg1m column
